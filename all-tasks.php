...
// Fetch all tasks with pagination (no filter)
$alltasks_query = "SELECT * FROM tasks ORDER BY created_at DESC LIMIT $limit OFFSET $offset";
$alltasks_result = mysqli_query($conn, $alltasks_query);
...
<body>
    ...
    <div class="sidebar">
        ...
        <ul class="sidebar-menu">
            ...
            <li><a href="all-tasks.php" class="active"><i class="fas fa-list"></i> All Tasks</a></li>
    ...
    <div class="main-content">
        ...
        <div class="welcome-header">
            <h2><i class="fas fa-tasks me-2"></i>All Tasks Management</h2>
            <p>View all tasks in the system (Total: <?php echo $totalTasks; ?>)</p>
        </div>

        <!-- Tasks Table -->
        <div class="table-section">
            <div class="table-header">
                <h4><i class="fas fa-clipboard-list me-2"></i>Tasks List</h4>
            </div>

            <?php if (mysqli_num_rows($alltasks_result) > 0): ?>
                <div class="table-responsive">
                    <table class="table table-hover">
                        ...
                        <tbody>
                            <?php while ($task = mysqli_fetch_assoc($alltasks_result)): ?>
                                <tr>
                                    ...
                                    <td class="text-center">
                                        <button type="button" class="btn btn-sm btn-info view-task-btn" 
                                            data-name="<?php echo htmlspecialchars($task['task_name'], ENT_QUOTES, 'UTF-8'); ?>" 
                                            data-desc="<?php echo htmlspecialchars($task['description'], ENT_QUOTES, 'UTF-8'); ?>" 
                                            data-created="<?php echo date('M d, Y', strtotime($task['created_at'])); ?>" 
                ...
                <?php if ($totalPages > 1): ?>
                    <nav aria-label="Tasks Pagination">
                        <ul class="pagination pagination-modern justify-content-center">
                            <?php if ($page > 1): ?>
                                <li class="page-item">
                                    <a class="page-link" href="all-tasks.php?page=<?php echo ($page - 1); ?>">
                            ...
                            <?php for ($i = 1; $i <= $totalPages; $i++): ?>
                                <li class="page-item <?php echo ($i == $page) ? 'active' : ''; ?>">
                                    <a class="page-link" href="all-tasks.php?page=<?php echo $i; ?>"><?php echo $i; ?></a>
                                </li>
                            <?php endfor; ?>
            ...
            <?php else: ?>
                <div class="text-center py-5">
                    <div class="icon-box mx-auto mb-4" style="width: 80px; height: 80px;">
                        <i class="fas fa-tasks text-white fs-1"></i>
                    </div>
                    <h3 class="fw-semibold text-dark mb-2">No Tasks Found</h3>
                    <p class="text-muted mb-4">There are no tasks in the system.</p>
                </div>
            <?php endif; ?>
        </div>
    ...
    <script>
        // Fix modal issue - use event delegation and proper modal initialization
        document.addEventListener('DOMContentLoaded', function() {
            ...
            document.addEventListener('click', function(e) {
                if (e.target.classList.contains('view-task-btn') || e.target.closest('.view-task-btn')) {
                    const button = e.target.classList.contains('view-task-btn') ? e.target : e.target.closest('.view-task-btn');
            ...
            modalElement.addEventListener('hidden.bs.modal', function() {
                document.getElementById('admin_view_task_name').textContent = '';
                document.getElementById('admin_view_description').textContent = '';
                document.getElementById('admin_view_created_at').textContent = '';
                document.getElementById('admin_view_status').innerHTML = '';
                document.getElementById('admin_view_image').src = '';
                document.getElementById('admin_view_image_container').style.display = 'none';
            });
        });
    </script>
